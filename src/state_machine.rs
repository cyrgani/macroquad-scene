use crate::{Node, RefMut};
use macroquad::experimental::coroutines::Coroutine;

type UpdateFn<T> = Box<dyn FnMut(&mut RefMut<T>, f32)>;
type CoroutineFn<T> = Box<dyn FnMut(&mut RefMut<T>) -> Coroutine>;
type OnEndFn<T> = Box<dyn FnMut(&mut RefMut<T>)>;
...
impl<T> State<T> {
    ...
    pub fn update(self, update: impl FnMut(&mut RefMut<T>, f32) + 'static) -> Self {
        State {
    ...
    pub fn coroutine(self, coroutine: impl FnMut(&mut RefMut<T>) -> Coroutine + 'static) -> Self {
        State {
    ...
    pub fn on_end(self, on_end: impl FnMut(&mut RefMut<T>) + 'static) -> Self {
        State {
...
impl<T: Node + 'static> StateMachine<T> {
    ...
    pub fn update_detached<'a, F>(mut t: RefMut<T>, mut f: F)
    where
        F: FnMut(&mut RefMut<T>) -> &mut StateMachine<T>,
    {
        ...
        let mut t = crate::get_node(handle);
...
impl<T: 'static> StateMachineOwned<T> {
    ...
    fn update(&mut self, player: &mut RefMut<T>, dt: f32) {
